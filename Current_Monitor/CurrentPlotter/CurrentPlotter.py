...
class CurrentPlotter:
    ...
    def create_widgets(self):
        ...
        self.checkbox_arduino = tk.Checkbutton(
            self.top_frame,
            text="Enable Arduino",
            variable=self.enable_arduino,
        ...
        self.interval_combo = ttk.Combobox(
            self.top_frame, values=["1 s", "1 min", "10 min", "1 hour"]
        )
        ...
        self.right_frame = tk.Frame(self.bottom_frame, width=150)
        ...
        self.label_name_unit_pairs = [("• Current", "A")]
        self.name_labels = []
        self.value_labels = []
        for i, (name, unit) in enumerate(self.label_name_unit_pairs):
            name_label, value_label = self.create_value_labels(
                name, unit, self.data_frame, i
            )
        ...
        self.current_time_label = self.create_value_labels(
            "Current Time", "", self.status_frame, 0
        )[1]
        self.arduino_status_label = self.create_value_labels(
            "Arduino stat", "", self.status_frame, 1
        )[1]
    ...
    def resize_figure(self):
        new_font_size = max(8, self.width // 75)
    ...
    def main_loop(self):
        ...
        if (
            loop_start_time - self.arduino_deque.get_last_1min_time().timestamp()
            < expected_exc_delay
        ):
            if self.get_interval() == Interval.ONE_MINUTE:
                self.update_plot()
            self.save_log(
                self.arduino_deque.get_last_1min_time(),
                self.arduino_deque.get_last_data(),
            )

        if (
            loop_start_time - self.arduino_deque.get_last_10min_time().timestamp()
            < expected_exc_delay
        ):
        ...
        if (
            loop_start_time - self.arduino_deque.get_last_1hour_time().timestamp()
            < expected_exc_delay
        ):
    ...
    def get_data_from_arduino(self):
        ...
        try:
            ...
            print(f"Connection error fetching from Arduino: {e}")
        except requests.exceptions.Timeout as e:
            self.arduino_status_code = "Timeout"
            print(f"Timeout error fetching from Arduino: {e}")
        except requests.exceptions.HTTPError as e:
            self.arduino_status_code = "HTTPError"
            print(f"HTTP error fetching from Arduino: {e}")
        except requests.exceptions.RequestException as e:
            self.arduino_status_code = "RequestException"
            print(f"General error fetching from Arduino: {e}")
        except Exception as e:
            self.arduino_status_code = "Critical"
            print(f"Critical error fetching from Arduino: {e}")
        return [0]
    ...
    def update_display(self):
        ...
        for i, position in enumerate(self.last_positions):
            ...
            self.value_labels[i].config(
                text=(
                    f": {self.arduino_deque.get_last_data()[data_order[position]]:.2f} "
                    f"{self.label_name_unit_pairs[position][1]}"
                )
            )
        ...
        self.arduino_status_label.config(
            text=(
                ...
                else self.make_error_sentence(self.arduino_status_code)
            )
        )
    ...
    def update_plot(self):
        ...
        self.ax.plot(
            self.time_arduino_plot,
            self.data_arduino_plot[0],
            marker="o",
            color="blue",
            label="Current",
    ...
    def save_log(self, time, arduino_data):
        ...
        year_month_dir = os.path.join(base, "log_current", year, month)
        ...
        with open(log_file_path, "a", encoding="utf-8") as f:
            f.write(
                f"{time.strftime('%Y-%m-%d %H:%M:%S')}: {arduino_data[0]:.2f} A\n"
            )


def resource_path(relative_path):
    """Resolve bundled icon assets next to exe / script."""
